...
export default async function CarsPage() {
  const listings = await prisma.listing.findMany({
    where: { status: "ACTIVE", category: { in: ["LUXURY_CARS", "HYPERCARS", "CLASSIC_CARS", "MOTORCYCLES"] } },
    take: 24,
    ...
    <div className="min-h-screen bg-[#050505] pt-20">
      <div className="bg-[#030303] border-b border-[rgba(201,168,76,0.1)] py-12 px-6">
        <div style={{maxWidth:1280,margin:"0 auto"}}>
          ...
          <h1 style={{fontFamily:"Cormorant Garamond,serif",fontSize:"clamp(2rem,5vw,3.5rem)",fontWeight:300,color:"#F0EAD0"}}>Luxury Cars & Hypercars</h1>
      ...
      <div style={{maxWidth:1280,margin:"0 auto",padding:"48px 24px"}}>
        {listings.length === 0 ? (
          <div style={{textAlign:"center",padding:"80px 0"}}>
            ...
            <a href="/listings/new"><button style={{marginTop:20,padding:"12px 32px",background:"linear-gradient(135deg,#C9A84C,#8B6914)",color:"#fff",border:"none",fontFamily:"Montserrat,sans-serif",fontSize:10,letterSpacing:"0.2em",textTransform:"uppercase",cursor:"pointer"}}>List Your Car</button></a>
          </div>
        ) : (
